...
unsigned my_hash_function(const struct hash_elem *e, void *aux){
	struct my_hash_elem *elem = hash_entry(e, struct my_hash_elem, hash_elem);
...
void hash_triple(struct hash_elem *e, void *aux){
	struct my_hash_elem *elem = hash_entry(e, struct my_hash_elem, hash_elem);
	elem->data = elem->data * elem->data * elem->data;
}

bool hash_less (const struct hash_elem *a, const struct hash_elem *b, void *aux){
	unsigned x = my_hash_function(a, aux);
	unsigned y = my_hash_function(b, aux);
	if(x<y)	return true;
	else return false;
}
