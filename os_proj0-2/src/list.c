...
/* Finds a run, starting at A and ending not after B, of list
  ...
  run. A through B (exclusive) must form a non-empty range. */
...
inplace_merge (struct list_elem *a0, struct list_elem *a1b0, struct list_elem *b1, list_less_func *less, void *aux){
  ...
  while (a0 != a1b0 && a1b0 != b1){
    if (!less (a1b0, a0, aux)) a0 = list_next (a0);
    else{
      a1b0 = list_next (a1b0);
...
void list_shuffle(struct list *list){
  ...
  for(int i=0; i < size*3; ++i){
    ...
    if(x != y){
      struct list_elem *list_elem1 = list_head(list);
      struct list_elem *list_elem2 = list_head(list);
      for(int j=0; j<=x; j++) list_elem1 = list_elem1->next;
      for(int j=0; j<=y; j++) list_elem2 = list_elem2->next;
      list_swap(list_elem1,list_elem2);
    }
  }
}
