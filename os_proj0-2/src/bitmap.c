...
void bitmap_set (struct bitmap *b, size_t idx, bool value){
  ...
  if (value) bitmap_mark (b, idx);
  else bitmap_reset (b, idx);
}
...
struct bitmap *bitmap_expand(struct bitmap *bitmap, int size){
	size_t original = bitmap_size(bitmap);
	bitmap->bit_cnt = original + size;
	bitmap->bits = realloc(bitmap->bits, byte_cnt(bitmap->bit_cnt));

	if(!bitmap->bits) return NULL;
	bitmap_set_multiple(bitmap, original, size, false);
	return bitmap;
}