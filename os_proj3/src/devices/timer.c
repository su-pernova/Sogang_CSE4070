...
void timer_sleep(int64_t ticks){
  ...
  enum intr_level old_level = intr_disable();
  struct list_elem *element = &current_thread->elem;

  current_thread->wakeuptime = start+ticks;
  list_push_back(&blocked_queue, element);
  thread_block();
...
static void timer_interrupt(struct intr_frame *args UNUSED){
  ...
  /* Proj 3 implementation */
  struct list_elem *element = list_begin(&blocked_queue);
  struct thread *thread = list_entry(element, struct thread, elem);
  while(element != list_end(&blocked_queue)){
    if(ticks >= thread->wakeuptime){
  ...
  if(thread_prior_aging || thread_mlfqs){
	  ...
	  if(timer_ticks() % TIMER_FREQ == 0) new_nice_recent_cpu();
	  if(timer_ticks() % 4 == 0) new_priority();
  }
